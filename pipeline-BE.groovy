pipeline {
    agent { label '' }
    environment {
        registry = "docker.io/ruum/mern-be"
        serviceName= 'mern-be'
        dockerImage = ''
        branchname = "main"
    }
    stages {
        ...
        stage('Deploy to Kubernetes') {
            steps{
                script {
                        sh "sed -i 's/mern-be:latest/mern-be:$env.VERSION/g' kubernetes/staging/deployment.yaml"
                        // withCredentials([file(credentialsId: '12fccb37-bc5b-4c54-aaec-c84dfd88c392', variable: 'KUBECRED')]) {
                        //     sh 'cat $KUBECRED > /var/lib/jenkins/.kube/config'
                        // }
                        sh "kubectl apply -f kubernetes/staging/deployment.yaml"
                        sh "kubectl rollout status deployment mern-be -n staging"
                        sh "kubectl get pods -n staging | grep mern-be"
                }
            }
        }
        stage('Approval') {
        ...
        stage('Deploy to Production') {
            steps{
                script {
                        sh "sed -i 's/mern-be:latest/mern-be:$env.VERSION/g' kubernetes/production/deployment.yaml"
                        sh "kubectl apply -f kubernetes/production/deployment.yaml"
                        sh "kubectl rollout status deployment mern-be -n production"
                        sh "kubectl get pods -n production | grep mern-be"
                    }
            }
        }
    }
